// @ts-nocheck
/**
 * Enhanced Seanime Extension for Comix.to
 * - Adds pagination to search (fetches up to 3 pages = ~84 results)
 * - Includes all content ratings in API calls
 * - Adds AniList ID resolution fallback
 * - Keeps the official API signature system
 */

// [RC4 key, mutKey, prefKey] x 5 rounds - KEEP THESE EXACTLY AS IS
const COMIX_KEYS = [
...
function rc4(key, data) {
    if (key.length === 0) return new Uint8Array(data);
...
function opShiftRight7Left1(value) { return ((value >>> 7) | (value << 1)) & 0xff; }
function opShiftLeft1Right7(value) { return ((value << 1) | (value >>> 7)) & 0xff; }
function opShiftRight2Left6(value) { return ((value >>> 2) | (value << 6)) & 0xff; }
function opShiftLeft4Right4(value) { return ((value << 4) | (value >>> 4)) & 0xff; }
function opShiftRight4Left4(value) { return ((value >>> 4) | (value << 4)) & 0xff; }
...
function mutateRound(data, mutKeyIndex, prefKeyIndex, prefLength, round) {
    ...
    for (let i = 0; i < data.length; i++) {
        if (i < prefLength && i < prefKey.length) output.push(prefKey[i]);
        ...
        switch (round) {
            case 1:
                switch (mode) {
                    ...
                    case 5: case 8: value = opShiftRight4Left4(value); break;
            ...
            case 2:
                switch (mode) {
                    ...
                    case 6: case 8: value = (value + 159) & 0xff; break;
            ...
            case 3:
                switch (mode) {
                    ...
                    case 2: case 9: value = opShiftLeft4Right4(value); break;
            ...
            case 4:
                switch (mode) {
                    case 0: case 7: value ^= 218; break;
                    case 1: case 4: value = opShiftLeft1Right7(value); break;
                    case 2: value = opShiftRight7Left1(value); break;
                    case 3: value = (value + 159) & 0xff; break;
                    case 5: case 8: value ^= 180; break;
            ...
            case 5:
                switch (mode) {
                    ...
                    case 1: case 3: value ^= 147; break;
                    case 2: value = (value + 34) & 0xff; break;
                    case 4: case 9: value ^= 218; break;
                    case 5: case 7: value = opShiftLeft1Right7(value); break;
                    case 6: value ^= 180; break;
                    case 8: value = opShiftRight2Left6(value); break;
                } break;
        }
...
function applyRound(data, rc4KeyIndex, mutKeyIndex, prefKeyIndex, prefLength, round) {
    return rc4(getKeyBytes(rc4KeyIndex), mutateRound(data, mutKeyIndex, prefKeyIndex, prefLength, round));
}
...
function generateComixHash(path) {
    const encoded = encodeURIComponent(path).replace(/\+/g, "%20").replace(/\*/g, "%2A").replace(/%7E/g, "~");
    const bytes = strToAsciiBytes(encoded);
    return bytesToUrlB64NoPad(round5(round4(round3(round2(round1(bytes))))));
}
...
class Provider {
    constructor() {
        ...
        this.MAX_SEARCH_PAGES = 3; // Fetch up to 3 pages for better results
        this.EXACT_MATCH_THRESHOLD = 0.9;
        this.FUZZY_MATCH_THRESHOLD = 0.7;
    }
    ...
    async fetchJson(url) {
        try {
            const response = await fetch(url, { headers: { Referer: this.api + "/" } });
    ...
    async fetchInBatches(items, batchSize, fn) {
        ...
        for (let i = 0; i < items.length; i += batchSize) {
            ...
            const batchResults = await Promise.all(batch.map(async (item) => {
                try { return await fn(item); } catch (e) { return []; }
            }));
    ...
    extractTitleSlug(url) {
        ...
        const marker = "/title/";
        const slug = value.indexOf(marker) >= 0 ? value.slice(value.indexOf(marker) + marker.length) : value.replace(/^\/?title\//, "").replace(/^\/+/, "");
        return slug.split(/[/?#]/)[0] || "";
    }
    ...
    normalizeMangaId(mangaId) {
        ...
        return { hashId, slug, fullSlug: slug ? hashId + "-" + slug : hashId };
    }
    ...
    // === NEW: Fuzzy title matching for fallback ===
    ...
    // === NEW: AniList ID resolution ===
    async resolveAnilistId(anilistId) {
        try {
            // Try direct search with AniList ID as keyword
            const url = this.apiUrl + "/manga?keyword=" + encodeURIComponent("anilist:" + anilistId) + "&limit=10&page=1";
    ...
    async search(opts) {
        ...
        const anilistTitle = opts && opts.anilistTitle ? opts.anilistTitle : null;
        const results = [];
        const seenIds = {};

        // Fetch multiple pages for better coverage
        for (let page = 1; page <= this.MAX_SEARCH_PAGES; page++) {
            const url = this.apiUrl + "/manga?keyword=" + encodeURIComponent(queryParam) + "&order[relevance]=desc&limit=28&page=" + page;
            try {
                const data = await this.fetchJson(url);
                ...
                for (let i = 0; i < items.length; i++) {
                    const item = items[i];
                    const hashId = item.hid || item.hash_id;
                    if (!hashId || seenIds[hashId]) continue;
                    seenIds[hashId] = true;

                    const slug = this.extractSlugFromItem(item, hashId);
                    const compositeId = hashId + "|" + slug;
                    const title = item.title || slug || hashId;

                    // If we have an anilistTitle, do fuzzy matching
                    if (anilistTitle) {
                        const altTitles = item.altTitles || item.alt_titles || [];
                        const allTitles = [title].concat(this.normalizeSynonyms(altTitles));
                        let bestScore = 0;
                        for (let j = 0; j < allTitles.length; j++) {
                            const score = this.calculateSimilarity(anilistTitle, allTitles[j]);
                    ...
                    results.push({
                        ...
                        _matchScore: anilistTitle ? this.calculateSimilarity(anilistTitle, title) : 1.0
                    });
                }

                // Stop if we got enough exact matches
                if (page === 1 && results.length > 0 && results[0]._matchScore >= this.EXACT_MATCH_THRESHOLD) {
                    break;
                }
            } catch (e) { break; }
        }

        // Sort by match score if we did fuzzy matching
        if (anilistTitle) {
            results.sort(function(a, b) { return (b._matchScore || 0) - (a._matchScore || 0); });
            // Return only top results if we have an exact match
            if (results.length > 0 && results[0]._matchScore >= this.EXACT_MATCH_THRESHOLD) {
                return [results[0]];
            }
            // Remove internal score field
            for (let i = 0; i < results.length; i++) { delete results[i]._matchScore; }
        }

        return results.slice(0, 20); // Limit to 20 results max
    }
    ...
    async findChapters(mangaId) {
        ...
        try {
            ...
            for (let page = 2; page <= totalPages; page++) { remainingPages.push(page); }

            const pageResults = await this.fetchInBatches(remainingPages, this.chapterPageConcurrency, (page) => this.fetchChapterItems(hashId, fullSlug, page, token));
            pageResults.forEach((items) => { if (items.length > 0) allChapters.push.apply(allChapters, items); });

            const chapters = [];
            allChapters.forEach((item) => {
                if (item.language && item.language.toLowerCase() !== "en" && item.language.toLowerCase() !== "english") return;
                const chapterId = item.id != null ? item.id : item.chapter_id;
                const chapterNumber = item.number != null ? this.formatChapterNumber(item.number) : (item.chapter || item.chap || "");
                if (!chapterId || !chapterNumber) return;
                const chapterTitle = item.name && item.name.trim().length > 0 ? "Chapter " + chapterNumber + ": " + item.name : "Chapter " + chapterNumber;
                ...
                const scanlator = group && group.name ? group.name.trim() : (isOfficial ? "Official" : undefined);
                chapters.push({
    ...
    async findChapterPages(chapterId) {
        ...
        const rawChapterId = parts.length >= 3 ? parts[2] : String(chapterId || "").split("/").pop();
        ...
        try {
            ...
            return images.filter((img) => img && img.url).map((img, index) => ({
                url: img.url,
                index: index,
